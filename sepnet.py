...
def train():
    ...
    with tf.name_scope('src_cross_entropy'):
        with tf.name_scope('total'):
            src_cross_entropy = tf.losses.sparse_softmax_cross_entropy(
    ...
    with tf.name_scope('tgt_cross_entropy'):
        with tf.name_scope('total'):
            tgt_cross_entropy = tf.losses.sparse_softmax_cross_entropy(
    ...
    with tf.name_scope('difference'):
        with tf.name_scope('total'):
            sfeature_src -= tf.reduce_mean(sfeature_src, 0)
            pfeature_src -= tf.reduce_mean(pfeature_src, 0)
            sfeature_src = tf.nn.l2_normalize(sfeature_src, 1)
            pfeature_src = tf.nn.l2_normalize(pfeature_src, 1)

            sfeature_tgt -= tf.reduce_mean(sfeature_tgt, 0)
            pfeature_tgt -= tf.reduce_mean(pfeature_tgt, 0)
            sfeature_tgt = tf.nn.l2_normalize(sfeature_tgt, 1)
            pfeature_tgt = tf.nn.l2_normalize(pfeature_tgt, 1)

            cor_mat_src = tf.matmul(sfeature_src, pfeature_src, transpose_a=True)
            cor_mat_tgt = tf.matmul(sfeature_tgt, pfeature_tgt, transpose_a=True)
